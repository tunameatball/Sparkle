...
{
    SULog(SULogLevelError, "Start Download with Request");
    NSLog("Start Download with Request");
...
{
    SULog(SULogLevelError, "Persistant Download");
    NSLog("Persistant Download");
...
{
    SULog(SULogLevelError, "TEMP Download");
    NSLog("TEMP Download");
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self->_download == nil && self->_delegate != nil) {
            // Prevent service from automatically terminating while downloading the update asynchronously without any reply blocks
